...
for i, text_file in enumerate(text_files, start=1):
    ...
    for paragraph in paragraphs:
        ...
        # Create the prompt
        prompt = f"A photograph in the style of {selected_artist}, depicting: '{paragraph}'"
        ...
        try:
            ...
            if previous_prompt == "":
                ...
                prompt = f"A photograph in the style of {selected_artist}, depicting: '{fallback_prompt}'"
                try:
                    response = openai.Image.create(
                    ...
                    prompt = f"A photograph in the style of {selected_artist}, depicting a random scene"
                    response = openai.Image.create(
        ...
        image_path = os.path.join(image_folder_path, f"{text_file.split('.')[0]}_image_{image_number}.png")
